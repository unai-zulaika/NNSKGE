...
def linf1(parameter, bias=None, reg=0.01, lr=0.1):
    ...
    sorted_z = torch.cat(
        (sorted_w_and_b, torch.zeros(rows, 1,
                                     device=torch.device(devicetype))), 1)
    subtracted = torch.clamp(sorted_w_and_b - sorted_z[:, 1:],
                             max=Norm)  #Max=Norm important

    scale_indices = torch.cumsum(
        torch.ones(rows, cols, device=torch.device(devicetype)), 1)
    ...
    shifted_ones = torch.cat(
        (torch.ones(rows, 1, device=torch.device(devicetype)),
         nonzero_ones[:, :(cols - 1)]), 1)
    over_one = -1 * (nonzero_ones - shifted_ones)
    last_one = torch.cat(
        (over_one, torch.zeros(rows, 1, device=torch.device(devicetype))),
        1)[:, 1:]
    max_remain = last_one * nonzero
    shift_max = torch.cat((torch.zeros(
        rows, 1, device=torch.device(devicetype)), max_remain[:, :(cols - 1)]),
                          1)
    first_col_nonzero_ones = torch.cat((torch.ones(
        rows, 1, device=torch.device(devicetype)), nonzero_ones[:, 1:]),
                                       1)  #Edge case for only first column
    ...
    nastyflipagainS = torch.flip(torch.flip(aggsubS, [0, 1]),
                                 [0])  #TODO: Edge cases

    updated_weights = sorted_w_and_b - nastyflipagainS
    unsorted = torch.zeros(rows, cols,
                           device=torch.device(devicetype)).scatter_(
                               1, indices, updated_weights)
...
def linf(parameter, bias=None, reg=0.01, lr=0.1):
    ...
    if bias is not None:
        w_and_b = torch.squeeze(
            torch.cat((parameter, bias.unfold(0, 1, 1)), 1), 0)
    else:
...
def l1(parameter, bias=None, reg=0.01, lr=0.1):
    ...
    if parameter.is_cuda:
        Norms_w = Norm * torch.ones(parameter.size(),
    ...
    if bias is not None:
        if bias.is_cuda:
            Norms_b = Norm * torch.ones(bias.size(),
                                        device=torch.device("cuda"))
        else:
            Norms_b = Norm * torch.ones(bias.size(),
                                        device=torch.device("cpu"))
...
def elasticnet(parameter, bias=None, reg=0.01, lr=0.1, gamma=1.0):
    ...
    if gamma < 0.0:
        print(
            "Warning, gamma should be positive. Otherwise you are not shrinking."
        )
